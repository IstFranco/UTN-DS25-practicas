        ...
        <div className='p-6 flex justify-center'>
            <form onSubmit={handleSubmit(onSubmit)} className='bg-[#b4978e] p-6 rounded shadow w-80 text-sm text-gray-900 space-y-4'>
                ...
                <div>
                    ...
                    <input type='text' id='usuario' {...register("usuario")} className={`w-full mt-1 p-2 rounded border border-gray-900 ${errors.usuario ? 'input-error' : ''}`} />
                ...
                <div>
                    ...
                    <input type='text' id='nombre' {...register("nombre")} className={`w-full mt-1 p-2 rounded border border-gray-900 ${errors.nombre ? 'input-error' : ''}`} />
                    {errors.nombre && <span className="field-error">{errors.nombre.message}</span>}
                </div>

                <div>
                    <label htmlFor='email' className='block font-medium'>Email</label>
                    <input type='email' id='email' {...register("email")} className={`w-full mt-1 p-2 rounded border border-gray-900 ${errors.email ? 'input-error' : ''}`} />
                    {errors.email && <span className="field-error">{errors.email.message}</span>}
                </div>

                <div>
                    <label htmlFor='password' className='block font-medium'>Contraseña</label>
                    <input type='password' id='password' {...register("password")} className={`w-full mt-1 p-2 rounded border border-gray-900 ${errors.password ? 'input-error' : ''}`} />
                ...
                <button type='submit' disabled={isSubmitting} className='bg-[#3d1f12] text-white w-full py-2 rounded hover:bg-[#2c140d] transition'>
